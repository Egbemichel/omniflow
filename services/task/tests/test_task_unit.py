...
class TestSubmissions:
    ...
    def test_create_submission_returns_201(self, client, end_user_headers, mock_workflow_service):
    ...
    def test_submission_creates_task_for_step_one(self, client, end_user_headers, mock_workflow_service):
        response = client.post("/submissions", json={
            "workflow_id": "wf-001",
            "form_data": {"field_1": "value_1"}
        }, headers=end_user_headers)
        ...
        tasks = client.get("/tasks/inbox", headers={"X-User-Id": "nurse-001", "X-User-Role": "nurse"})
        task_ids = [t["submission_id"] for t in tasks.json()]
        assert submission_id in task_ids

    def test_create_submission_missing_workflow_id_returns_422(self, client, end_user_headers):
    ...
    def test_user_cannot_view_other_users_submissions(self, client, created_submission):
        ...
        response = client.get(f"/submissions/{sub_id}/status", headers=other_user_headers)
        assert response.status_code == 403


class TestTaskInbox:

    def test_staff_sees_tasks_assigned_to_their_role(self, client, nurse_headers, created_submission):
    ...
    def test_doctor_does_not_see_nurse_tasks(self, client, doctor_headers, created_submission):
...
class TestTaskCompletion:
    ...
    def test_complete_task_advances_workflow(self, client, nurse_headers, created_submission, mock_workflow_service):
    ...
    def test_complete_task_appends_audit_history(self, client, nurse_headers, created_submission, mock_workflow_service):
        tasks = client.get("/tasks/inbox", headers=nurse_headers)
        task_id = tasks.json()[0]["id"]
        sub_id = created_submission["id"]
    ...
    def test_wrong_role_cannot_complete_task(self, client, doctor_headers, created_submission):
        """Step 1 is assigned to nurse — doctor must not complete it."""
        nurse_tasks = client.get("/tasks/inbox",
            headers={"X-User-Id": "nurse-001", "X-User-Role": "nurse"}
        )
    ...
    def test_cannot_complete_already_completed_task(self, client, nurse_headers, created_submission, mock_workflow_service):
        tasks = client.get("/tasks/inbox", headers=nurse_headers)
        task_id = tasks.json()[0]["id"]
