...
def get_workflow(
    workflow_id: str, db: Session = Depends(get_db), role: str = Depends(_get_role)
):
...
@router.post(
    "/workflows/{workflow_id}/steps", response_model=schemas.StepOut, status_code=201
)
def add_step(
    workflow_id: str,
...
def initialise(
    ...
    if wf.status != "published":
        raise HTTPException(
            status_code=409, detail="Workflow must be published to initialise"
        )
...
def transition(
    ...
    if x_user_role != current_role and x_user_role != "admin":
        raise HTTPException(
            status_code=403, detail="Your role cannot advance this step"
        )
