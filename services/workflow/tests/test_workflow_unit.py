...
class TestWorkflowCRUD:
    ...
    def test_create_workflow_returns_201(
        self, client, admin_headers, sample_workflow_payload
    ):
    ...
    def test_non_admin_cannot_create_workflow(
        self, client, nurse_headers, sample_workflow_payload
    ):
        response = client.post(
            "/workflows", json=sample_workflow_payload, headers=nurse_headers
        )
        assert response.status_code == 403

    def test_publish_draft_workflow_changes_status(
        self, client, admin_headers, draft_workflow
    ):
    ...
    def test_cannot_publish_already_published_workflow(
        self, client, admin_headers, published_workflow
    ):
        wf_id = published_workflow["id"]
        response = client.post(f"/workflows/{wf_id}/publish", headers=admin_headers)
        assert response.status_code == 409

    def test_cannot_edit_steps_of_published_workflow(
        self, client, admin_headers, published_workflow
    ):
        wf_id = published_workflow["id"]
        response = client.post(
    ...
    def test_get_workflow_returns_full_definition(
        self, client, admin_headers, draft_workflow
    ):
        wf_id = draft_workflow["id"]
...
class TestWorkflowStateMachine:
    ...
    def test_initialise_creates_state_at_step_one(
        self, client, admin_headers, published_workflow
    ):
        wf_id = published_workflow["id"]
        response = client.post(
            f"/workflows/{wf_id}/initialise",
    ...
    def test_initialise_on_draft_workflow_returns_409(
        self, client, admin_headers, draft_workflow
    ):
        wf_id = draft_workflow["id"]
        response = client.post(
    ...
    def test_transition_advances_to_next_step(
        self, client, admin_headers, nurse_headers, published_workflow
    ):
        wf_id = published_workflow["id"]
        client.post(
            f"/workflows/{wf_id}/initialise",
    ...
    def test_wrong_role_cannot_advance_step(
        self, client, admin_headers, doctor_headers, published_workflow
    ):
        wf_id = published_workflow["id"]
        client.post(
            f"/workflows/{wf_id}/initialise",
    ...
    def test_completing_all_steps_marks_submission_completed(
        self, client, admin_headers, nurse_headers, doctor_headers, published_workflow
    ):
        wf_id = published_workflow["id"]
    ...
    def test_cannot_transition_completed_submission(
        self, client, admin_headers, nurse_headers, doctor_headers, published_workflow
    ):
        wf_id = published_workflow["id"]
        admin_step_headers = {"X-User-Id": "admin-001", "X-User-Role": "admin"}
    ...
    def test_transition_unknown_submission_returns_404(
        self, client, nurse_headers, published_workflow
    ):
        wf_id = published_workflow["id"]
        response = client.post(
            f"/workflows/{wf_id}/transition",
